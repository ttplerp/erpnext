...
def get_stock_balance(
    ...
    if with_valuation_rate:
        if with_serial_no:
            ...
            return (
                (last_entry.qty_after_transaction, last_entry.valuation_rate)
                if last_entry
                else (0.0, 0.0)
            )
    else:
...
def get_or_make_bin(item_code: str, warehouse: str) -> str:
    bin_record = frappe.db.get_value(
...
def _create_bin(item_code, warehouse):
    ...
    try:
        ...
        bin_obj = frappe.get_doc(
            doctype="Bin", item_code=item_code, warehouse=warehouse
        )
        bin_obj.flags.ignore_permissions = 1
        bin_obj.insert()
    except frappe.UniqueValidationError:
        frappe.db.rollback(
            save_point=bin_creation_savepoint
        )  # preserve transaction in postgres
...
def get_incoming_rate(args, raise_error_if_no_rate=True):
    ...
    if (args.get("serial_no") or "").strip():
        ...
        if valuation_method in ("FIFO", "LIFO", "SPECIFIC"):
            if previous_sle:
                ...
                in_rate = (
                    _get_fifo_lifo_rate(
                        previous_stock_queue, args.get("qty") or 0, valuation_method
                    )
    ...
    # As per tshering lham recommendation, i have excluded check of valaution rate in stock entry
    # get_item_form when the purpse is "Purchase/Requisition"
    if in_rate is None and args.get("purpose") != "Purchase/Requisition":
...
def get_valuation_method(item_code):
    ...
    if not val_method:
        val_method = (
            frappe.db.get_value("Stock Settings", None, "valuation_method", cache=True)
            or "FIFO"
        )
    return val_method
...
def validate_warehouse_company(warehouse, company):
    warehouse_company = frappe.db.get_value(
        "Warehouse", warehouse, "company", cache=True
    )
            ...
            columns.insert(
                idx + 1,
                {
                    ...
                    "fieldname": "{0}_{1}".format(
                        d.get("fieldname"), frappe.scrub(include_uom)
                    ),
                    "fieldtype": "Currency"
                    if d.get("convertible") == "rate"
                    else "Float",
                },
            )
...
def add_additional_uom_columns(columns, result, include_uom, conversion_factors):
    ...
    for row_idx, row in enumerate(result):
        for convertible_col, data in convertible_column_map.items():
            ...
            if for_type == "rate":
                row[data.converted_col] = (
                    flt(value_before_conversion) * conversion_factor
                )
            else:
                row[data.converted_col] = (
                    flt(value_before_conversion) / conversion_factor
...
def get_incoming_outgoing_rate_for_cancel(
    item_code, voucher_type, voucher_no, voucher_detail_no
):
...
def _update_item_info(
    scan_result: Dict[str, Optional[str]]
