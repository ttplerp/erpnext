		...
		for item in self.material_issue_details:
			...
			if flt(item.allocated_amount) > 0:
				...
				if flt(balance_amount) < flt(item.allocated_amount) and self.docstatus < 2:
					frappe.throw(_("Row #{0} : Allocated amount Nu. {1}/- cannot be more than  Outstanding Amount Nu. {2}/-").format(item.reference_name, "{:,.2f}".format(flt(item.allocated_amount)),"{:,.2f}".format(flt(balance_amount))))
				else:
	...
	def make_gl_entry(self, cancel=False):
		...
		if getattr(self, 'settle_imprest_advance', False):
			...
			for entry in gl_entries:
				# For Project Imprest account (11.3.003), always use expense branch cost center
				if entry.get('account') == "11.3.003 - Project Imprest":
					if expense_branch_cc:
