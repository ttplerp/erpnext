...
class JournalEntry(AccountsController):
		...
		for a in self.accounts:
			if a.reference_type == "Hire Charge Advance" and a.reference_name:
				if hire_charge_advance in [a.reference_name]:
					...
					hire_charge_advance[a.reference_name] = frappe._dict(
		...
		for key, value in hire_charge_advance.items():
			...
			if cancel:
				...
				if doc.payment_type == "Pay":
					...
					doc.journal_entry_status = "Received on {0}".format(
						now_datetime().strftime("%Y-%m-%d %H:%M:%S")
					)
	...
	def update_reference_document(self, cancel=False):
		if cancel:
			for a in self.get("accounts"):
				...
				if a.reference_type == "Supplier Advance Settlement" and a.reference_name:
			...
			for a in self.get("accounts"):
				...
				if a.reference_type == "Supplier Advance Settlement" and a.reference_name:
					doc = frappe.get_doc("Supplier Advance Settlement", a.reference_name)
					doc.db_set('journal_entry_status', "Settle on {0}".format(now_datetime().strftime("%Y-%m-%d %H:%M:%S")))
	...
	def set_total_debit_credit(self):
		...
		for d in self.get("accounts"):
			...
			self.total_credit = (
				flt(self.total_credit) + flt(d.credit, d.precision("credit")) + flt(tax_cr)
			)
						...
						self.get_gl_dict(
							{
								...
								"debit": flt(abs(tax_amount_dr), d.precision("tax_amount"))
								if tax_account
								else flt(d.debit, d.precision("debit")),
								"credit": flt(abs(tax_amount_cr), d.precision("tax_amount"))
								if tax_account
