...
class JournalEntry(AccountsController):
	...
	def update_hire_charge_advance(self, cancel=False):
		...
		for a in self.accounts:
			if a.reference_type == "Hire Charge Advance" and a.reference_name:
				if hire_charge_advance in [a.reference_name]:
					hire_charge_advance[a.reference_name]["credit"] += flt(a.credit)
					hire_charge_advance[a.reference_name]["debit"] += flt(a.debit)
				else:
					hire_charge_advance[a.reference_name] = frappe._dict({"credit": flt(a.credit), "debit": flt(a.debit)})

		factor = 1
		for key, value in hire_charge_advance.items():
			doc = frappe.get_doc("Hire Charge Advance", key)
			if cancel:
				factor = -1
		...
		for a in self.accounts:
			if a.reference_type == "MR Employee Advance" and a.reference_name:
				if mr_employee_advance in [a.reference_name]:
					...
					mr_employee_advance[a.reference_name] = frappe._dict({"credit": flt(a.credit), "debit": flt(a.debit)})
	...
	def update_transporter_invoice(self):
		frappe.db.sql(
			"""update `tabTransporter Invoice` ti
	...
	def validate_debit_credit_amount(self):
		for d in self.get("accounts"):
			if not flt(d.debit) and not flt(d.credit):
				frappe.throw(
					_("Row {0}: Both Debit and Credit values cannot be zero").format(
						d.idx
					)
				)
	...
	def set_total_debit_credit(self):
		...
		for d in self.get("accounts"):
			...
			self.total_debit = (
				flt(self.total_debit) + flt(d.debit, d.precision("debit")) + flt(tax_dr)
			)
			self.total_credit = (
				flt(self.total_credit)
	...
	def set_amounts_in_company_currency(self):
		for d in self.get("accounts"):
			...
			d.taxable_amount_in_account_currency = (
				flt(d.taxable_amount_in_account_currency)
				if flt(d.taxable_amount_in_account_currency)
				else (
					flt(d.debit_in_account_currency)
					or flt(d.credit_in_account_currency)
				)
			)
			d.taxable_amount_in_account_currency = flt(
				d.taxable_amount_in_account_currency,
				d.precision("taxable_amount_in_account_currency"),
			)
			d.taxable_amount = flt(
				d.taxable_amount_in_account_currency * flt(d.exchange_rate),
				d.precision("taxable_amount"),
			)
			...
			d.tax_amount_in_account_currency = flt(
				tax_amount, d.precision("tax_amount_in_account_currency")
			)
			d.tax_amount = flt(
				d.tax_amount_in_account_currency * flt(d.exchange_rate),
				d.precision("tax_amount"),
			)
	...
	def set_print_format_fields(self):
		...
		) = party_account_currency = pay_to_recd_from = None
		party_type = None
		for d in self.get("accounts"):
			...
			elif frappe.db.get_value("Account", d.account, "account_type") in [
				"Bank",
				"Cash",
			]:
				bank_amount += (
	...
	def build_gl_map(self):
		...
		for d in self.get("accounts"):
			if (d.debit or d.credit) or (
				...
				if (
					cint(self.apply_tds)
					and cint(d.apply_tds)
					and d.tax_account
					and flt(d.rate)
					and flt(d.tax_amount)
				):
				...
				for acc in with_tax:
					...
					(
						tax_amount_in_account_currency_dr,
						tax_amount_in_account_currency_cr,
					) = (0, 0)
					tax_amount_dr, tax_amount_cr = 0, 0

					if tax_account:
						tax_amount_in_account_currency = flt(
							d.tax_amount_in_account_currency
						)
						tax_amount = flt(d.tax_amount)

						if d.add_deduct_tax == "Add":
							tax_amount_in_account_currency_cr = (
								tax_amount_in_account_currency if flt(d.credit) else 0
							)
							tax_amount_in_account_currency_dr = (
								tax_amount_in_account_currency if flt(d.debit) else 0
							)
					...
					party_type = party = ""
					if get_account_type(acc, self.company) in [
						"Receivable",
						"Payable",
						"Expense Account",
						"Income Account",
						...
						self.get_gl_dict(
							{
								...
								"debit": flt(
									abs(tax_amount_dr), d.precision("tax_amount")
								)
								if tax_account
								else flt(d.debit, d.precision("debit")),
								"credit": flt(
									abs(tax_amount_cr), d.precision("tax_amount")
								)
								if tax_account
								else flt(d.credit, d.precision("credit")),
								"account_currency": d.account_currency,
								"debit_in_account_currency": flt(
									abs(tax_amount_in_account_currency_dr),
									d.precision("tax_amount_in_account_currency"),
								)
								if tax_account
								else flt(
								...
								"credit_in_account_currency": flt(
									abs(tax_amount_in_account_currency_cr),
									d.precision("tax_amount_in_account_currency"),
								)
								if tax_account
								else flt(
...
def get_payment_entry_against_invoice(
	...
	return get_payment_entry(
		ref_doc,
		{
			...
			"debit_in_account_currency": debit_in_account_currency,
			"remarks": "Payment received against {0} {1}. {2}".format(
