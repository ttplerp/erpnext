...
class PurchaseInvoice(BuyingController):
	...
	def validate(self):
		...
		# Overriding the total amount and supplier advance
		if self.supplier_advances:
			allocated_amount = 0.0
			for adv in self.supplier_advances:
				allocated_amount += flt(adv.allocated_amount)
			self.total_advance = flt(allocated_amount)

			self.outstanding_amount = flt(self.outstanding_amount) - flt(self.total_advance)
	...
	def check_prev_docstatus(self):
		for d in self.get("items"):
			...
			if d.purchase_receipt:
				...
				if not submitted:
					frappe.throw(
						_("Purchase Receipt {0} is not submitted").format(d.purchase_receipt)
					)
	...
	def update_tds_receipt_entry(self):
		if self.amended_from:
			tds_receipt_entry = frappe.db.get_value(
				"TDS Receipt Entry", {"invoice_no": self.amended_from}, "name"
			)
			if tds_receipt_entry:
				frappe.get_doc("TDS Receipt Entry", tds_receipt_entry).db_set(
	...
	def consume_budget(self, cancel=False):
		if cancel:
			frappe.db.sql(
				"delete from `tabCommitted Budget` where reference_type='{}' and reference_no='{}'".format(
		...
		for item in self.get("items"):
			...
			if item.po_detail:
				expense, cost_center = frappe.db.get_value(
					"Purchase Order Item",
					item.po_detail,
				...
				if frappe.db.get_value("Item", item.item_code, "is_fixed_asset"):
					expense = get_asset_category_account(
						"fixed_asset_account", item=item.item_code, company=self.company
					)
			...
			if bud_acc_dtl.centralized_budget:
				...
				budget_cost_center = (
					cc_doc.budget_cost_center if cc_doc.use_budget_from_parent else cost_center
				)
			if expense:
				if bud_acc_dtl.account_type in ("Fixed Asset", "Expense Account"):
					...
					amount = (
						...
						if flt(item.base_net_amount, 2)
						else flt(item.base_amount, 2)
					)
					if (
						frappe.db.get_single_value("Budget Settings", "budget_commit_on")
						== "Material Request"
					):
						...
						mr_child_id = frappe.db.get_value(
							"Material Request Item",
							{"parent": mr_name, "item_code": item.item_code},
							"name",
						)
						if mr_name:
							reference_date = (
								frappe.db.get_value("Material Request", mr_name, "creation_date")
								if mr_name
								else self.posting_date
							)
							commited_budget_id = frappe.db.get_value(
								"Committed Budget",
								{
									"reference_type": "Material Request",
									"reference_no": mr_name,
									"reference_id": mr_child_id,
								},
								"name",
							)
					else:
						...
						commited_budget_id = frappe.db.get_value(
							"Committed Budget",
							{
								...
								"reference_no": item.purchase_order,
								"reference_id": item.po_detail,
							},
							"name",
						)
					args = frappe._dict(
					...
					if amount == com_doc.amount and not com_doc.closed:
						frappe.db.sql(
							"update `tabCommitted Budget` set closed = 1 where name = '{}'".format(
								commited_budget_id
							)
						)
	...
	def make_gl_entries(self, gl_entries=None, from_repost=False):
		...
		if gl_entries:
			...
			if self.docstatus == 1:
				...
				provisional_entries = [
					a for a in gl_entries if a.voucher_type == "Purchase Receipt"
				]
	...
	def make_advance_gl_entry(self, gl_entries):
		for a in self.get("supplier_advances"):
			...
			if flt(item.base_net_amount):
				...
				if (
					...
					if flt(item.rm_supp_cost):
						supplier_warehouse_account = warehouse_account[self.supplier_warehouse][
							"account"
						]
				...
				# ):
					...
					if provisional_accounting_for_non_stock_items:
						if item.purchase_receipt:
							...
							purchase_receipt_doc = purchase_receipt_doc_map.get(
								item.purchase_receipt
							)
	...
	def make_stock_adjustment_entry(
		...
		if (
			...
			!= flt(
				voucher_wise_stock_value.get((item.name, item.warehouse)),
				net_amt_precision,
			)
		):
	...
	def make_gle_for_rounding_adjustment(self, gl_entries):
		...
		if (
			not self.is_internal_transfer()
			and self.rounding_adjustment
			and self.base_rounding_adjustment
		):
			(
				round_off_account,
				round_off_cost_center,
			) = get_round_off_account_and_cost_center(self.company, "Purchase Invoice", self.name)
...
@frappe.whitelist()
def post_journal_entry(source_name, target_doc=None):
	...
	def set_missing_values(source, target):
		...
		accounts = [
			{
				...
				"party_type": "Employee",
				"party": source.imprest_party,
				"reference_type": "Purchase Invoice",
				"reference_name": source.name,
			},
			{
				"account": source.credit_to,
		...
		for account in accounts:
			...
			row.balance = cc_acc_data.get("balance")
			row.party_type = account.get("party_type")
			row.party = account.get("party")
			row.reference_type = account.get("reference_type")
			row.reference_name = account.get("reference_name")
