...
class Account(NestedSet):
    ...
    def onload(self):
        ...
        if (
            not frozen_accounts_modifier
            or frozen_accounts_modifier in frappe.get_roles()
        ):
    ...
    def validate_root_details(self):
        ...
        if not self.parent_account and not self.is_group:
            frappe.throw(
                _("The root account {0} must be a group").format(frappe.bold(self.name))
            )

    def validate_root_company_and_sync_account_to_children(self):
        # ignore validation while creating new compnay or while syncing to child companies
        if (
            frappe.local.flags.ignore_root_company_validation
            or self.flags.ignore_root_company_validation
        ):
            return
        ...
        if ancestors:
            ...
            if not frappe.db.get_value(
                "Account",
                {"account_name": self.account_name, "company": ancestors[0]},
                "name",
            ):
                frappe.throw(
                    _("Please add the account to root level Company - {}").format(
                        ancestors[0]
                    )
                )
    ...
    def validate_frozen_accounts_modifier(self):
        ...
        if old_value and old_value != self.freeze_account:
            ...
            if (
                not frozen_accounts_modifier
                or frozen_accounts_modifier not in frappe.get_roles()
            ):
    ...
    def validate_account_currency(self):
        ...
        gl_currency = frappe.db.get_value(
            "GL Entry", {"account": self.name}, "account_currency"
        )
...
def update_account_number(
    name, account_name, account_number=None, from_descendant=False
):
    ...
    if ancestors and not allow_independent_account_creation:
        for ancestor in ancestors:
            if frappe.db.get_value(
                "Account", {"account_name": old_acc_name, "company": ancestor}, "name"
            ):
...
def merge_account(old, new, is_group, root_type, company):
    ...
    val = list(
        frappe.db.get_value("Account", new, ["is_group", "root_type", "company"])
    )
...
def sync_update_account_number_in_child(
    ...
    for d in frappe.db.get_values(
        ...
        update_account_number(
            d["name"], account_name, account_number, from_descendant=True
        )
