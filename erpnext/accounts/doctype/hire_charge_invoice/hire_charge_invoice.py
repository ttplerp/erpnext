...
class HireChargeInvoice(AccountsController):
    ...
    def calculate_totals(self):
        ...
        for d in self.get("advances"):
            if d.amount <= 0:
                frappe.throw("Advance amount should be more than zero")
            if not d.account:
                frappe.throw("Account is mandatory for deductions")
            total_deductions += flt(d.amount, 2)
    ...
    def advance_gl_entries(self, gl_entries):
    ...
    def make_item_gl_entries(self, gl_entries):
        expense_account = frappe.db.get_single_value(
            "Maintenance Settings", "equipment_hire_expense_account"
        )
    ...
    def get_logbooks(self):
        ...
        if len(entries) == 0:
            frappe.msgprint(
                "No valid logbooks found for owner {} between {} and {}.You must have pulled in other Hire Charge Invoices(including draft invoices)".format(
