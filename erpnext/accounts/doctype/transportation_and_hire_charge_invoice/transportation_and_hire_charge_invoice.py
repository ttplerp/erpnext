...
class TransportationandHireChargeInvoice(AccountsController):
    def validate(self):
    ...
    def update_logbook(self):
        for a in self.items:
            ...
            ehf_doc = frappe.get_doc(
                "Equipment Hiring Form", {"name": logbook.equipment_hiring_form}
            )
    ...
    def set_internal_cc_and_branch(self):
        if self.party_type == "Supplier":
            ...
            if cint(self.is_internal_customer) == 1:
                branch, cc = frappe.db.get_value(
                    "Customer", {"name": self.party}, ["branch", "cost_center"]
                )
        ...
        if self.party_type == "Supplier":
            ...
            if cint(self.is_internal_customer) == 1:
                income_account = frappe.db.get_single_value(
                    "Maintenance Settings", "equipment_hire_income_account"
                )
    ...
    def get_rate(self, ehf):
        ...
        if rate:
            ...
            throw(
                "No rates defined in Equipment Hiring Form <b>{}</b>".format(
                    frappe.get_desk_link("Equipment Hiring Form", ehf)
                ),
                title="Hiring Rate Not Found",
            )
    ...
    def calculate_totals(self):
            ...
            for a in self.items:
                ...
                if not a.expense_account and a.expense_head:
                    a.expense_account = frappe.db.get_value(
                        "Expense Head", a.expense_head, "expense_account"
                    )
                    if not a.expense_account:
                        throw(
                            "Expense Account not defined in Expense Head {}".format(
                                bold(a.expense_head)
                            ),
                            title="Expense Account Not Found",
                        )
        ...
        self.payable_amount = self.outstanding_amount = flt(
            self.grand_total - self.total_deduction, 2
        )
    ...
    def deduction_gl_entries(self, gl_entries):
        for d in self.deduct_items:
            party_type = party = ""
            if get_account_type(d.account, self.company) in [
                "Receivable",
                "Payable",
                "Expense Account",
                "Income Account",
    ...
    def make_item_gl_entries(self, gl_entries):
        ...
        for item in self.items:
            party_type = party = ""
            if get_account_type(expense_account, self.company) in [
                "Receivable",
                "Payable",
                "Expense Account",
                "Income Account",
    ...
    def make_tds_gl_entries(self, gl_entries):
        if flt(self.tds_amount) > 0:
            party_type = party = ""
            if get_account_type(self.tds_account, self.company) in [
                "Receivable",
                "Payable",
                "Expense Account",
                "Income Account",
    ...
    def make_supplier_gl_entry(self, gl_entries):
        if flt(self.payable_amount) > 0 and self.party_type == "Supplier":
            if self.invoice_type == "Hire Charge Invoice":
                payable_account = frappe.db.get_single_value("Maintenance Settings", "hire_charge_payable_account")
                ...
                payable_account = frappe.db.get_single_value("Maintenance Settings", "transportation_payable_account")
                if not payable_account:
                    frappe.throw("Plese set <strong>Transportation Payable Account </strong>in Maintenance Settings")

                expense_account = frappe.db.get_single_value("Maintenance Settings", "transportation_expense_account")
                if not expense_account:
                    frappe.throw("Plese set <strong>Transportation Payable Account </strong>in Maintenance Settings")
            if payable_account == expense_account:
    ...
    def get_logbooks(self):
        ...
        eme = qb.DocType("Transportation and Transportation and Hire Charge Invoice")
        ...
        if len(entries) == 0:
            frappe.msgprint(
                "No valid logbooks found for owner {} between {} and {}.You must have pulled in other Transportation and Hire Charge Invoices(including draft invoices)".format(
                    frappe.bold(self.party), frappe.bold(self.from_date), frappe.bold(self.to_date)
                ),
    ...
    def post_journal_entry(self):
        ...
        if (
            self.settle_imprest_advance == 1
            and self.party_type == "Supplier"
            and not imprest_advance_account
        ):
        ...
        je.update(
            {
                ...
                "title": "Transportation and Hire Charge Invoice " + self.party,
                "user_remark": "Note: " + "Hire Charge Payment - " + self.party
                if self.party_type == "Supplier"
                else "Note: " + "Hire Charge Receivable - " + self.party,
        ...
        if self.party_type == "Supplier":
            je.append(
                "accounts",
                {
                    ...
                    "party_type": self.party_type,
                    "party": self.party,
                    "reference_type": "Transportation and Hire Charge Invoice",
        ...
        frappe.msgprint(
            _("{0} posted to accounts").format(frappe.get_desk_link("Journal Entry", je.name))
        )
...
def get_permission_query_conditions(user):
    ...
    return """(
		`tabTransportation and Hire Charge Invoice`.owner = '{user}'
		or 
		exists(select 1
			from `tabEmployee` as e
			where e.branch = `tabTransportation and Hire Charge Invoice`.branch
			and e.user_id = '{user}')
		or
		exists(select 1
			...
			and bi.branch = `tabTransportation and Hire Charge Invoice`.branch)
	)""".format(
        user=user
    )
