...
def execute(filters=None):
	...
	if data:
		...
		# Create and update the "Margin" row
		create_and_update_row(data, "Margin", revenue_actuals, expense_actuals,
							  revenue_planned, expense_planned, comparison_revenue, comparison_expense)

		# Create and update the "Earnings Before Income Tax" row
		create_and_update_row(data, "Earnings Before Income Tax", revenue_actuals, expense_actuals,
							  revenue_planned, expense_planned, comparison_revenue, comparison_expense)
		
		# Create and update the "Tax" row
		create_tax_row(data, "Tax", revenue_actuals - expense_actuals, revenue_planned - expense_planned,
					   comparison_revenue - comparison_expense)

	return columns, data, parent_children_map

def create_tax_row(data, name, earnings_before_tax_actual, earnings_before_tax_planned, earnings_before_tax_comparison):
	tax_rate = 0.30  # 30% tax rate
	tax_amount_actual = earnings_before_tax_actual * tax_rate
	...
	row = {
		...
		"account": name,
		"indent": 0,
		"actuals": tax_amount_actual,
		"planned": tax_amount_planned,
		"actual_plan_difference": 0.0,
		"actual_plan_diff_percent": 0.0,
		"comparison_actuals": tax_amount_comparison,
	...
	row["actual_plan_difference"] = row["actuals"] - row["planned"]
	...
	row["comparison_actual_difference"] = round(row["actuals"] - row["comparison_actuals"], 2)
...
def create_and_update_row(data, name, revenue_actuals, expense_actuals, revenue_planned, expense_planned,
						  comparison_revenue, comparison_expense):
	row = {
		"account_name": name,
		"account": name,
		"indent": 0,
		"actuals": revenue_actuals - expense_actuals,
		"planned": revenue_planned - expense_planned,
		"actual_plan_difference": 0.0,
		"actual_plan_diff_percent": 0.0,
		"comparison_actuals": comparison_revenue - comparison_expense,
		"comparison_actual_difference": 0.0,
		"comparison_actual_diff_percent": 0.0,
		"has_value": True,
	}
