...
# name changed for use in key indicators report
def get_key_data(
	company,
	root_type,
	balance_must_be,
...
def get_columns(periodicity, period_list, accumulated_values=1, company=None):
	...
	for period in period_list:
		label = f"{period['from_date']} and {period['to_date']}"  # Construct the label with date ranges
		columns.append(
			{
	...
	# Additional columns for planned amounts and balanced amounts
	columns.append(
		{
			"fieldname": "planned_amount",
			"label": _("Planned Amount"),
			"fieldtype": "Currency",
			"width": 150
		}
	)
	columns.append(
		{
			"fieldname": "balanced_amount",
			"label": _("Balanced Amount"),
			"fieldtype": "Currency",
			"width": 150
		}
	)
