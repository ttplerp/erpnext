...
def get_payment_entries(filters):
    return frappe.db.sql(
		...
		select
			...
			( 
                if(pe.paid_from=%(account)s, pe.paid_amount, 0)
                + ifnull(pe.total_taxes_and_charges, 0) - ifnull(pe.total_advance_amount, 0)
            ) as credit,
...
def get_loan_entries(filters):
    ...
    for doctype in ["Loan Disbursement", "Loan Repayment"]:
        ...
        if doctype == "Loan Repayment" and frappe.db.has_column(
    ...
    for doctype in ["Loan Disbursement", "Loan Repayment"]:
        ...
        query = (
            ...
            .where(
                ...
                <= getdate(filters.get("report_date"))
            )
        )
