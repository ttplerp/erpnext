...
def execute(filters=None):
	...
	# Auto fetch default company if not provided
...
def get_data(filters):
	...
	# ✅ Added company filter here
	gl_entries = frappe.db.sql(
		"""
		SELECT
			posting_date, debit, credit, is_opening, party_type, party, cost_center,
			voucher_no, company
		FROM `tabGL Entry`
		WHERE
			is_cancelled = 0
			...
			AND cost_center IN (
				SELECT name FROM `tabCost Center` WHERE cost_center_for = %(cost_center_for)s
			)
	...
	for key, value in data.items():
		...
		if filter_data["closing_debit"] > filter_data["closing_credit"]:
			filter_data["closing_debit"] = flt(
				filter_data["closing_debit"] - filter_data["closing_credit"], 2
			)
			filter_data["closing_credit"] = 0
		elif filter_data["closing_debit"] < filter_data["closing_credit"]:
			filter_data["closing_credit"] = flt(
				filter_data["closing_credit"] - filter_data["closing_debit"], 2
			)
			filter_data["closing_debit"] = 0
		else:
			filter_data["closing_debit"] = 0
...
def get_columns(filters):
	if filters.get("party_type") == "Employee":
		columns = [
			...
			{"label": _("Employee Name"), "fieldname": "employee_name", "fieldtype": "Data", "width": 100},
		]
	else:
		columns = [
			{"label": _("Supplier"), "fieldname": "supplier", "fieldtype": "Data", "width": 160},
			{"label": _("TPN"), "fieldname": "tpn", "fieldtype": "Data", "width": 100},
		]

	columns.extend(
		[
			{"label": _("Opening Debit"), "fieldname": "opening_debit", "fieldtype": "Float", "width": 120},
			{"label": _("Opening Credit"), "fieldname": "opening_credit", "fieldtype": "Float", "width": 120},
			{"label": _("Debit"), "fieldname": "debit", "fieldtype": "Float", "width": 120},
			{"label": _("Credit"), "fieldname": "credit", "fieldtype": "Float", "width": 120},
			{"label": _("Closing Debit"), "fieldname": "closing_debit", "fieldtype": "Float", "width": 120},
			{"label": _("Closing Credit"), "fieldname": "closing_credit", "fieldtype": "Float", "width": 120},
		]
	)
	return columns
