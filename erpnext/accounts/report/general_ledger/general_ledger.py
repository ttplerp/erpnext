...
def get_columns(filters):
	...
	columns = [
		...
		{"label": _("Branch"), "options": "Branch", "fieldname": "cost_center", "width": 120},
...
# 	# First, initialize branch for all entries with their own cost_center
# 	for d in data:
# 		d["branch"] = d.get("cost_center", "")

# 	# Get all GL entries for the vouchers in our filtered data to find pairs
# 	voucher_nos = set()
# 	for d in data:
# 		if d.get("voucher_no"):
# 			voucher_nos.add(d.get("voucher_no"))

# 	if voucher_nos:
# 		# Fetch all GL entries for these vouchers to find complete pairs
# 		all_voucher_entries = frappe.db.sql("""
# 			SELECT voucher_no, account, cost_center, party, party_type
# 			FROM `tabGL Entry` 
# 			WHERE voucher_no IN %s AND company=%s
# 			ORDER BY voucher_no, creation
# 		""", (list(voucher_nos), filters.get("company")), as_dict=1)

# 		# Group by voucher_no to find pairs
# 		voucher_map = {}
# 		for entry in all_voucher_entries:
# 			voucher_no = entry.get("voucher_no")
# 			if voucher_no not in voucher_map:
# 				voucher_map[voucher_no] = []
# 			voucher_map[voucher_no].append(entry)

# 		# Apply branch swapping for complete pairs
# 		for voucher_no, entries in voucher_map.items():
# 			if len(entries) == 2:
# 				cost_center_1 = entries[0].get("cost_center")
# 				cost_center_2 = entries[1].get("cost_center")
				
# 				# Update branch in our filtered data for both entries
# 				for d in data:
# 					if d.get("voucher_no") == voucher_no:
# 						if d.get("cost_center") == cost_center_1:
# 							d["branch"] = cost_center_2
# 						elif d.get("cost_center") == cost_center_2:
# 							d["branch"] = cost_center_1
...
# 		{"label": _("Party"), "fieldname": "party", "width": 100},
# 		{"label": _("Project"), "options": "Project", "fieldname": "project", "width": 100},
# 		{"label": _("Cost Center"), "options": "Cost Center", "fieldname": "branch", "width": 120},
# 		{"label": _("Branch"), "options": "Cost Center", "fieldname": "cost_center", "width": 120},
# 	]
