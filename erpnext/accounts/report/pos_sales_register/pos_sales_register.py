...
def execute(filters=None):
	...
	# cash, online, wire transfer
	pos_entries = update_pos_entries(pos_entries)
...
def get_pos_entries(filters, group_by_field):
	...
	# if group_by_field == "mode_of_payment":
	select_mop_field = ", sip.mode_of_payment, sip.base_amount"
	...
	return frappe.db.sql(
		...
		SELECT
			...
			p.customer, p.is_return, p.jrnal_no {select_mop_field}
		FROM
...
def update_pos_entries(pos_entries):
	data, row = {}, []
	for d in pos_entries:
		data.setdefault(d.get("pos_invoice"), []).append(d)
	...
	for key, value in data.items():
		...
		for d in value:
			new_data['posting_date'] = d.posting_date
			new_data['pos_profile'] = d.pos_profile
			new_data['owner'] = d.owner
			new_data['jrnal_no'] = d.jrnal_no
			new_data['is_return'] = d.is_return
			...
			if cstr(d.mode_of_payment) == 'Wire Transfer':
				new_data['wire_transfer'] = new_data['wire_transfer'] + d.base_amount
				new_data['grand_total'] = new_data['grand_total'] + new_data['wire_transfer']
