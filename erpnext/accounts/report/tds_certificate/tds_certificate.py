...
def get_datax(filters):
	...
	if filters.customer_type == "Supplier" and filters.supplier:
		...
		query1 = """SELECT  
						...
						as tds_amount,
						(select cheque_number from `tabTDS Receipt Entry` rr where rr.purchase_invoice = d.name limit 1) as cheque_number,
						(select cheque_date from `tabTDS Receipt Entry` rr where rr.purchase_invoice = d.name limit 1) as cheque_date,
						(select receipt_number from `tabTDS Receipt Entry` rr where rr.purchase_invoice = d.name limit 1) as receipt_number,
						(select receipt_date from `tabTDS Receipt Entry` rr where rr.purchase_invoice = d.name limit 1) as receipt_date
					FROM 
					...
					AND 
						di.party = '{2}'
					AND EXISTS(select 1 from `tabTDS Receipt Entry` rr where rr.purchase_invoice = d.name and supplier = '{2}')
					AND d.currency = '{3}'
		...
		query = """SELECT 
					...
					INNER JOIN
						`tabTDS Receipt Entry` AS rr ON rr.purchase_invoice = b.name
		...
		query1 = """SELECT i.transaction_date, i.payable_amount as tds_taxable_amount, 
					...
					INNER JOIN
					`tabTDS Receipt Entry` AS rr ON rr.purchase_invoice = i.parent 
...
def get_journal_entries(filters):
	return frappe.db.sql("""
			...
			INNER JOIN `tabTDS Receipt Entry` AS rr ON rr.purchase_invoice = t.name
