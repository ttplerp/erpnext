...
def validate_fiscal_year(date, fiscal_year, company, label="Date", doc=None):
    ...
    if fiscal_year not in years:
        if doc:
            ...
            throw(
                _("{0} '{1}' not in Fiscal Year {2}").format(label, formatdate(date), fiscal_year)
            )
...
def get_balance_on(
    ...
    if from_date is None:
        ...
        cond.append(
            "posting_date BETWEEN '{0}' AND '{1}' ".format(from_date, date if date else nowdate())
        )
    ...
    if cost_center and report_type == "Profit and Loss":
        ...
        if cc.is_group:
            ...
            cond.append(
                """gle.cost_center = %s """ % (frappe.db.escape(cost_center, percent=False),)
            )
...
def check_clearance_date(dt, dn):
    ...
    if doc.clearance_date:
        frappe.msgprint(
            "Cannot cancel this document as <b>Bank Reconciliation</b> has already done on {}".format(
                doc.clearance_date
            ),
...
def reconcile_against_document(args, update_outstanding="Yes"):  # nosemgrep
    ...
    for key, entries in reconciled_entries.items():
        ...
        create_payment_ledger_entry(
            gl_map, update_outstanding=update_outstanding, cancel=1, adv_adj=1
        )
        ...
        gl_map = doc.build_gl_map()
        create_payment_ledger_entry(
            gl_map, update_outstanding=update_outstanding, cancel=0, adv_adj=1
        )
...
def check_if_advance_entry_modified(args):
    ...
    if args.voucher_type == "Journal Entry":
        ret = frappe.db.sql(
			...
			and (t2.reference_type is null or t2.reference_type in ('', 'Sales Order', 'Purchase Order', 'POL Expense'))
...
def unlink_ref_doc_from_payment_entries(ref_doc):
    ...
    if ref_doc.doctype in ("Sales Invoice", "Purchase Invoice"):
        ...
        frappe.db.sql(
            """delete from `tab{0} Advance` where parent = %s""".format(ref_doc.doctype),
            ref_doc.name,
        )
    ...
    for d in invoice_list:
        ...
        if outstanding_amount > 0.5 / (10**precision):
            if (
                ...
                and max_outstanding
                and not (
                    outstanding_amount >= min_outstanding and outstanding_amount <= max_outstanding
                )
            ):
                continue
...
def get_tds_account(percent, company, party_type=None):
    ...
    if party_type == "Customer":
        ...
        if not account:
            frappe.throw("Please set Receivable Account in Company TDS and Retention Table")
    else:
        account = frappe.db.get_value(
...
def get_autoname_with_number(number_value, doc_title, name, company, doctype=None):
...
def update_voucher_outstanding(voucher_type, voucher_no, account, party_type, party):
    ...
    if (
        ...
        frappe.db.set_value(
            ...
            voucher_no,
            "outstanding_amount",
            outstanding["outstanding_in_account_currency"],
        )
        ...
        if cint(pol_exp.use_common_fuelbook) == 1:
            total_debit = frappe.db.get_value(
                "Journal Entry", pol_exp.journal_entry, "total_debit"
            )
            ref_doc = frappe.get_doc(voucher_type, voucher_no)
            # Didn't use db_set for optimisation purpose
