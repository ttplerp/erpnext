...
class CustomWorkflow:
	def __init__(self, doc):
		...
		if self.doc.doctype != "Material Request" and self.doc.doctype != "Performance Evaluation" and self.doc.doctype not in ("Project Capitalization","Asset Issue Details", "Compile Budget","POL Expense","Vehicle Request", "Repair And Services"):
			...
			if not self.expense_approver:
				if not frappe.db.get_value("Employee", self.doc.employee, "expense_approver"):
					frappe.throw("Expense Approver not set for Employee {0}.",format(self.doc.employee))
				if not frappe.db.get_value("Employee", {"user_id":frappe.db.get_value("Employee", self.doc.employee, "expense_approver")}, self.field_list):
					frappe.throw('{1} set as Expense Approver for Employee {0} is not an Employee'.format(self.doc.employee, frappe.db.get_value("Employee", self.doc.employee, "expense_approver")))
			...
			self.dept_approver = frappe.db.get_value("Employee",{"user_id":frappe.db.get_value(
					"Department Approver",
					{"parent": str(frappe.db.get_value("Employee", self.doc.employee, "department")), "parentfield": "expense_approvers", "idx": 1},
					"approver",
				)},self.field_list)
	...
	def leave_application(self):
		...
		if self.new_state.lower() in ("Draft".lower(),"Waiting Supervisor Approval".lower()):
			if "HR User" in frappe.get_roles(frappe.session.user):
				...
				if frappe.session.user != frappe.db.get_value("Employee",self.doc.employee,"user_id") and self.old_state.lower() != self.new_state.lower():
	...
	def overtime_application(self):
		if self.new_state.lower() in ("Draft".lower(), "Waiting Supervisor Approval".lower()):
			...
			if self.doc.approver != frappe.session.user:
				frappe.throw("Only {} can Forward/Edit this request".format(self.doc.approver_name))
			self.set_approver("Department Head")
		elif self.new_state.lower() in ('Rejected'.lower()):
			if self.doc.approver != frappe.session.user:
