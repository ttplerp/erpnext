...
class CustomWorkflow:
    def __init__(self, doc):
        ...
        if (
            ...
            self.reports_to = frappe.db.get_value(
                "Employee",
                {"name": frappe.db.get_value("Employee", self.doc.employee, "reports_to")},
                self.field_list,
            )
            ...
            self.dept_approver = frappe.db.get_value(
                ...
                frappe.db.get_value(
                    "Department",
                    str(frappe.db.get_value("Employee", self.doc.employee, "department")),
                    "approver",
                ),
                self.field_list,
            )
            self.gm_approver = frappe.db.get_value(
                ...
                frappe.db.get_value(
                    ...
                    {
                        "department_name": str(
                            frappe.db.get_value("Employee", self.doc.employee, "division")
                        )
                    },
                    "approver",
                ),
            ...
            if self.doc.doctype in [
                ...
                if self.doc.doctype == "Leave Application":
                    ...
                    self.project_head = frappe.db.get_value(
                        ...
                        frappe.db.get_value(
                            "Employee",
                            {"user_id": self.doc.leave_approver},
                            "reports_to",
                        ),
                        self.field_list,
                    )
                ...
                if self.doc.doctype == "Employee Transfer Request":
                    ...
                    if not self.supervisor:
                        frappe.throw(
                            "Plesse set Leave Approver for employee {} in Employee List".format(
                                self.employee
                            )
                        )
                    self.hrgm = frappe.db.get_value(
        ...
        if self.doc.doctype == "Performance Evaluation":
            ...
            self.eval_2 = frappe.db.get_value(
                "Employee",
                frappe.db.get_value("Employee", self.doc.employee, "eval_3"),
                self.field_list,
            )
            self.eval_3 = frappe.db.get_value(
                "Employee",
                frappe.db.get_value("Employee", self.doc.employee, "eval_3"),
                self.field_list,
            )
        ...
        if self.doc.doctype == "Material Request":
            self.mechanical_cs = frappe.db.get_value(
                "Employee",
                {"user_id": frappe.db.get_single_value("Maintenance Settings", "mechanical_cs")},
                self.field_list,
            )
            ...
            if not self.hrgm:
                frappe.throw("HR GM Approver not set in HR setting")
            ...
            self.reports_to = frappe.db.get_value(
                "Employee",
                frappe.db.get_value("Employee", {"user_id": self.doc.owner}, "reports_to"),
                self.field_list,
            )

            if not self.reports_to:
                frappe.throw("Reports to not set for the employee: {}".format(self.doc.owner))
    ...
    def set_approver(self, approver_type):
        if approver_type == "Supervisor":
            ...
            vars(self.doc)[self.doc_approver[0]] = officiating[0] if officiating else self.qhse[0]
            vars(self.doc)[self.doc_approver[1]] = officiating[1] if officiating else self.qhse[1]
            vars(self.doc)[self.doc_approver[2]] = officiating[2] if officiating else self.qhse[2]
        elif approver_type == "PPE Approver":
            officiating = get_officiating_employee(self.ppe_approver[3])
            if officiating:
                officiating = frappe.db.get_value(
                    "Employee", officiating[0].officiate, self.field_list
                )
            vars(self.doc)[self.doc_approver[0]] = (
                officiating[0] if officiating else self.ppe_approver[0]
            )
            vars(self.doc)[self.doc_approver[1]] = (
                officiating[1] if officiating else self.ppe_approver[1]
            )
            vars(self.doc)[self.doc_approver[2]] = (
                officiating[2] if officiating else self.ppe_approver[2]
            )
        ...
        elif approver_type == "Advance Approver":
            officiating = get_officiating_employee(self.advance_approver[3])
            if officiating:
                officiating = frappe.db.get_value(
                    "Employee", officiating[0].officiate, self.field_list
                )
            vars(self.doc)[self.doc_approver[0]] = (
                officiating[0] if officiating else self.advance_approver[0]
            )
            vars(self.doc)[self.doc_approver[1]] = (
                officiating[1] if officiating else self.advance_approver[1]
            )
            vars(self.doc)[self.doc_approver[2]] = (
                officiating[2] if officiating else self.advance_approver[2]
            )
            ...
            officiating = get_officiating_employee(self.project_head[3])
            if officiating:
                officiating = frappe.db.get_value(
                    "Employee", officiating[0].officiate, self.field_list
                )
            vars(self.doc)[self.doc_approver[0]] = (
                officiating[0] if officiating else self.project_head[0]
            )
            vars(self.doc)[self.doc_approver[1]] = (
                officiating[1] if officiating else self.project_head[1]
            )
            vars(self.doc)[self.doc_approver[2]] = (
            ...
            vars(self.doc)[self.doc_approver[0]] = (
                officiating[0] if officiating else self.budget_reappropiation_approver[0]
            )
            vars(self.doc)[self.doc_approver[1]] = (
                officiating[1] if officiating else self.budget_reappropiation_approver[1]
            )
            vars(self.doc)[self.doc_approver[2]] = (
                officiating[2] if officiating else self.budget_reappropiation_approver[2]
            )
        else:
    ...
    def apply_workflow(self):
        ...
        elif self.doc.doctype == "Transportation Charge":
            self.transportation_charge()
    ...
    def compile_budget(self):
        if not self.old_state:
            ...
            self.old_state.lower() == "Draft".lower() and self.new_state.lower() != "Draft".lower()
        ):
            if (
                ...
                if self.doc.budget_level == "Region":
                    ...
                    approver = frappe.db.get_value(
                        "Employee",
                        frappe.db.get_value("Department", self.doc.division, "approver"),
                        self.field_list,
                    )
            ...
            self.old_state.lower() in ("Waiting RD Approval".lower(), "Rejected by GMO".lower())
            ...
            approver = frappe.db.get_value(
                "Employee",
                frappe.db.get_value("Department", "Operations Division - BTL", "approver"),
                self.field_list,
            )
            officiating = get_officiating_employee(approver[3])
            if officiating:
    ...
    def target_setup_request(self):
        if self.new_state.lower() in ("Draft".lower()):
            ...
            if self.doc.approver != frappe.session.user:
                frappe.throw(
                    "Only <b>{}</b> can Approve/Reject this Target".format(self.doc.approver_name)
                )
    ...
    def performance_evaluation(self):
        ...
        if self.new_state.lower() in ("Draft".lower()):
            ...
            if self.doc.approver != frappe.session.user:
                frappe.throw(
                    "Only {} can Forward/Reject this Target".format(self.doc.approver_name)
                )
    ...
    def employee_benefits(self):
        if self.new_state.lower() in ("Draft".lower(), "Waiting GM Approval".lower()):
            ...
            if self.doc.benefit_approver != frappe.session.user:
                frappe.throw(
                    "Only {} can edit/submit this document".format(self.doc.benefit_approver_name)
                )
    ...
    def coal_raising_payment(self):
        if self.new_state.lower() in (
            ...
            if "Production User" not in frappe.get_roles(frappe.session.user):
                if self.doc.approver != frappe.session.user:
                    frappe.throw(
                        "Only {} can edit/submit this documents".format(self.doc.approver)
                    )
        ...
        if self.new_state.lower() in ("Waiting Verification".lower()):
            if frappe.session.user != self.doc.owner:
                if (
                    ...
                    frappe.throw(
                        "Only {} can forward this Asset for verification.".format(self.doc.owner)
                    )
                if (
    ...
    def leave_application(self):
        ...
        if self.new_state.lower() == "Draft".lower():
            ...
            if not self.doc.supervisor_remarks:
                frappe.throw("Supervisor remarks required.")
            ...
            if not self.doc.approver_remarks:
                frappe.throw("Approver remarks required.")
            ...
            if (
                self.doc.leave_approver != frappe.session.user
                and self.new_state.lower() != self.old_state.lower()
            ):
                frappe.throw(
                    "Only {} can Reject this Leave Application".format(
                        self.doc.leave_approver_name
                    )
                )
            if self.doc.workflow_state == "Recommended By Supervisor".lower() and not self.doc.approver_remarks:
                frappe.throw("Rejection requires remarks from the approver. Please provide your comments.")

            if self.old_state.lower() == "Waiting Supervisor Approval".lower() and not self.doc.supervisor_remarks:
            ...
            if "HR User" not in frappe.get_roles(frappe.session.user):
                frappe.throw(
                    _("Only {} can Cancel this Leave Application").format(
                        self.doc.leave_approver_name
                    )
                )
    ...
    def employee_transfer_request(self):
        # Waiting Supervisor Approval -> Waiting Approval
        if self.new_state.lower() in ("Waiting Supervisor Approval".lower()):
            self.set_approver("Supervisor")
        ...
        if self.new_state.lower() in ("Waiting Approval".lower()):
            if self.doc.supervisor != frappe.session.user:
                frappe.throw(
                    "Only {} can forward this Employee Transfer Request".format(
            ...
            if self.doc.supervisor != frappe.session.user:
                frappe.throw(
                    "Only {} can Approve this Employee Transfer Request".format(
                        self.doc.supervisor_name
                    )
                )
            ...
            if (
                ...
                frappe.throw(
                    "Only {} can Reject this Employee Transfer Request".format(
                        self.doc.supervisor_name
                    )
                )
            ...
            if "HR User" not in frappe.get_roles(frappe.session.user):
                frappe.throw(
                    _("Only {} can Cancel this Employee Transfer Request").format(
                        self.doc.supervisor_name
                    )
                )
        ...
        if self.new_state.lower() in ("Waiting CEO Approval".lower()):
            if "Director" not in frappe.get_roles(
                ...
                if self.doc.advance_approver != frappe.session.user:
                    frappe.throw(
                        _("Only {} can Verify this request").format(self.doc.advance_approver_name)
                    )
            self.set_approver("CEO")
            ...
            if self.doc.advance_approver != frappe.session.user:
                frappe.throw(
                    _("Only {} can Approve this request").format(self.doc.advance_approver_name)
                )
            self.doc.db_set("status", self.new_state)
        elif self.new_state.lower() == "Rejected":
            if self.doc.advance_approver != frappe.session.user:
                frappe.throw(
                    _("Only {} can Reject this request").format(self.doc.advance_approver_name)
                )
            self.doc.db_set("status", self.new_state)
        elif self.new_state.lower() == "Cancelled".lower():
            if frappe.session.user not in (self.doc.advance_approver, "Administrator"):
                frappe.throw(
                    _("Only {} can Cancel this document.").format(self.doc.advance_approver_name)
                )
            self.doc.db_set("status", self.new_state)
    ...
    def employee_advance(self):
        ...
        #     self.set_approver("Supervisor")
            ...
            if self.doc.advance_approver != frappe.session.user:
                frappe.throw("Only {} can Cancel this request".format(self.doc.advance_approver_name))
    ...
    def budget_reappropiation(self):
        ...
        if self.new_state.lower() in ("Rejected".lower()):
            if (
                ...
                frappe.throw(
                    "Only Budget Manager or {} Can reject this document".format(self.doc.approver)
                )
    ...
    def material_request(self):
        ...
        if self.new_state.lower() == "Draft".lower():
            ...
            if self.old_state.lower() == "Waiting Central Store approval".lower():
                ...
                if self.doc.approver != frappe.session.user:
                    frappe.throw(
                        "Only {} can Approve the Material Request".format(self.doc.approver_name)
                    )
    ...
    def festival_advance(self):
        ...
        if self.new_state.lower() in (
            ...
            if self.doc.advance_approver != frappe.session.user:
                frappe.throw(
                    "Only {} can Approve the Festival Claim".format(self.doc.advance_approver_name)
                )
    ...
    def assign_shift(self):
        if self.new_state.lower() in ("Draft".lower()):
            ...
            if self.doc.approver:
                if self.doc.approver != frappe.session.user:
                    frappe.throw(
                        "Only {} can approve or edit this document.".format(self.doc.approver)
                    )
            self.set_approver("GM")
...
class NotifyCustomWorkflow:
    ...
    def notify_employee(self):
        ...
        if self.doc.doctype == "Transportation Charge":
            template = frappe.db.get_single_value(
                "HR Settings", "transportation_charge_status_notification"
            )
            if not template:
                frappe.msgprint(
                    _(
                        "Please set default template for Transportation Charge Status Notification in HR Settings."
                    )
                )
                return
        ...
        if self.doc.doctype == "Leave Application":
            ...
            template = frappe.db.get_single_value(
                "HR Settings", "travel_request_status_notification_template"
            )
            if not template:
                frappe.msgprint(
                    _(
                        "Please set default template for Travel Request Status Notification in HR Settings."
                    )
                )
                return
        elif self.doc.doctype == "Overtime Application":
            ...
            template = frappe.db.get_single_value("HR Settings", "pol_receive_status_notification")
            if not template:
                frappe.msgprint(
                    _(
                        "Please set default template for POL Receive Status Notification in HR Settings."
                    )
                )
                return
        elif self.doc.doctype == "Material Request":
            ...
            template = frappe.db.get_single_value(
                "HR Settings", "vehicle_request_status_notification_template"
            )
            if not template:
                frappe.msgprint(
                    _(
                        "Please set default template for Vehicle Request Status Notification in HR Settings."
                    )
                )
                return
    ...
    def notify_approver(self):
        if self.doc.get(self.doc_approver[0]):
            ...
            if self.doc.doctype == "Leave Application":
                ...
                template = frappe.db.get_single_value(
                    "HR Settings", "transportation_charge_approval_notification"
                )
                if not template:
                    frappe.msgprint(
                        _(
                            "Please set default template for Transportation Charge Approval Notification in HR Settings."
                        )
                    )
                    return
                ...
                template = frappe.db.get_single_value(
                    "HR Settings",
                    "employee_transfer_approval_notification_template",
                )
                if not template:
                    frappe.msgprint(
                        _(
                ...
                template = frappe.db.get_single_value(
                    "HR Settings", "travel_request_approval_notification_template"
                )
                if not template:
                    frappe.msgprint(
                        _(
                            "Please set default template for Travel Request Approval Notification in HR Settings."
                        )
                    )
                    return
            elif self.doc.doctype == "Overtime Application":
            ...
            #         return
                ...
                template = frappe.db.get_single_value(
                    "HR Settings", "pol_receive_approval_notification"
                )
                if not template:
                    frappe.msgprint(
                        _(
                            "Please set default template for POL Receive Approval Notification in HR Settings."
                        )
                    )
                    return
            elif self.doc.doctype == "Repair And Services":
    ...
    def send_notification(self):
        ...
        if self.new_state == "Draft":
            ...
            and self.doc.doctype not in ("Asset Issue Details", "Project Capitalization")
        ):
