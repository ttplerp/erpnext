...
frappe.ui.form.on('Asset Finance Book', {
	asset_sub_category:function(frm, cdt, cdn){
		const row = locals[cdt][cdn];
		frappe.call({
			method: "erpnext.assets.doctype.asset.asset.get_finance_book_details",
			args: {
				"asset_sub_category": row.asset_sub_category
			},
			callback: function(r) {
				row.depreciation_method = r.message[0];
				row.total_number_of_depreciations = r.message[1];
				row.frequency_of_depreciation = r.message[2];
				row.depreciation_start_date=r.message[3];
				row.income_depreciation_percent = r.message[4];
				frm.refresh();
			}
		})
	},
