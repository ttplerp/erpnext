...
class Asset(AccountsController):
    ...
    def fetch_default_account(self):
        self.asset_account, self.accumulated_depreciation_account = frappe.db.get_value(
            "Asset Category Account",
            {"parent": self.asset_category},
    ...
    def validate_asset_and_reference(self):
        if self.purchase_invoice or self.purchase_receipt:
            reference_doc = (
                "Purchase Invoice" if self.purchase_invoice else "Purchase Receipt"
            )
            ...
            if reference_doc.get("company") != self.company:
                frappe.throw(
                    ...
                    ).format(self.name, reference_doc.get("name"))
                )
    ...
    def validate_item(self):
        ...
        if not item:
            ...
            frappe.throw(
                _("Item {0} must be a Fixed Asset Item").format(self.item_code)
            )
    ...
    def set_missing_values(self):
        ...
        if not self.asset_sub_category:
            self.asset_sub_category = frappe.db.get_value(
                "Item", self.item_code, "asset_sub_category"
            )
        if self.item_code and not self.get("finance_books"):
            finance_books = get_item_details(
                self.item_code, self.asset_category, self.asset_sub_category
            )
            self.set("finance_books", finance_books)
        if not self.abbr:
            self.abbr = frappe.db.get_value(
                "Asset Category", self.asset_category, "abbr"
            )
        if not self.branch:
            self.branch = frappe.db.get_value(
                "Branch", {"cost_center": self.cost_center}, "branch"
            )
    ...
    def validate_asset_values(self):
        ...
        if is_cwip_accounting_enabled(self.asset_category):
            if not self.is_existing_asset and not (
                self.purchase_receipt or self.purchase_invoice
            ):
                frappe.throw(
                    _(
        ...
        if self.available_for_use_date and getdate(
    ...
    def _make_depreciation_schedule(self, finance_book, start, date_of_sale):
        ...
        number_of_pending_depreciations = cint(
        ...
        skip_row = False
        should_get_last_day = is_last_day_of_the_month(
        ...
        for n in range(start[finance_book.idx - 1], number_of_pending_depreciations):
            ...
            if not has_pro_rata or n < cint(number_of_pending_depreciations) - 1:
                ...
                monthly_schedule_date = add_months(
                    schedule_date, -finance_book.frequency_of_depreciation + 1
                )
            ...
            if n == 0:
                ...
                depreciation_amount = get_depreciation_amount(
                    self, value_after_depreciation, finance_book, schedule_date, days
                )
            ...
            income_depreciation_amount = self._get_income_tax_depreciation_amount(
                finance_book, schedule_date, days, income_accumulated_depreciation
            )
            ...
            value_after_depreciation -= flt(
                depreciation_amount, self.precision("gross_purchase_amount")
            )
            # Adjust depreciation amount in the last period based on the expected value after useful life
            if finance_book.expected_value_after_useful_life and (
                (
    ...
    def _get_income_tax_depreciation_amount(
        self, finance_book, schedule_date, no_of_days, income_accumulated_depreciation
    ):
        dep_per_year = (
            flt(self.gross_purchase_amount)
            - flt(finance_book.expected_value_after_useful_life)
        ) * (flt(finance_book.income_depreciation_percent) / 100)
        days_in_year = (
            date_diff(
                get_year_ending(getdate(schedule_date)),
                get_year_start(getdate(schedule_date)),
            )
            + 1
        )
        income_depreciation_amount = (flt(dep_per_year) / cint(days_in_year)) * cint(
            no_of_days
        )
        if (
            flt(self.gross_purchase_amount)
            - flt(finance_book.expected_value_after_useful_life)
        ) >= (flt(income_accumulated_depreciation) + income_depreciation_amount):
    ...
    def get_adjusted_depreciation_amount(
        ...
        if not self.opening_accumulated_depreciation:
            depreciation_amount_for_first_row = (
                self.get_depreciation_amount_for_first_row(finance_book)
            )
    ...
    def set_accumulated_depreciation(
        ...
        for i, d in enumerate(self.get("schedules")):
            ...
            if int(d.finance_book_id) not in finance_books:
                value_after_depreciation = flt(
                    self.get_value_after_depreciation(d.finance_book_id)
                )
            ...
            depreciation_amount = flt(
                d.depreciation_amount, d.precision("depreciation_amount")
            )
    ...
    def validate_expected_value_after_useful_life(self):
        for row in self.get("finance_books"):
            ...
            if accumulated_depreciation_after_full_schedule:
                ...
                if (
                    row.expected_value_after_useful_life
                    and row.expected_value_after_useful_life
    ...
    def validate_make_gl_entry(self):
        ...
        query = (
            """SELECT name FROM `tabGL Entry` WHERE voucher_no = %s and account = %s"""
        )
        if asset_bought_with_invoice:
            ...
            cwip_booked = frappe.db.sql(
                query, (purchase_document, cwip_account), as_dict=1
            )
            if cwip_booked:
        ...
        if self.is_existing_asset and self.asset_category != "Land" and days > 30:
            ...
            je.append(
                "accounts",
                {
                    ...
                    "credit": self.opening_accumulated_depreciation,
                    "reference_type": "Asset",
                    "reference_name": self.name,
                    "cost_center": self.cost_center,
                },
            ...
            je.append(
                "accounts",
                {
                    ...
                    "debit": self.opening_accumulated_depreciation,
                    "reference_type": "Asset",
                    "reference_name": self.name,
                    "cost_center": self.cost_center,
                },
    ...
    def get_depreciation_rate(self, args, on_validate=False):
        ...
        if args.get("depreciation_method") == "Written Down Value":
            ...
            depreciation_rate = math.pow(
                value, 1.0 / flt(args.get("total_number_of_depreciations"), 2)
            )
...
def get_asset_account(account_name, asset=None, asset_category=None, company=None):
    ...
    if not account:
        if not asset_category:
            frappe.throw(
                _("Set {0} in company {1}").format(
                    account_name.replace("_", " ").title(), company
                )
            )
        else:
            frappe.throw(
...
def get_depreciation_amount(
    asset, depreciable_value, row, schedule_date, no_of_days_in_a_schedule
):
    if row.depreciation_method in ("Straight Line", "Manual"):
        ...
        depreciation_amount = (
            flt(asset.gross_purchase_amount) - flt(row.expected_value_after_useful_life)
        ) / (flt(row.total_number_of_depreciations) / 12)
        depreciation_amount = flt(depreciation_amount) * (
            flt(no_of_days_in_a_schedule) / flt(days_in_year)
        )
    else:
        depreciation_amount = flt(
            depreciable_value * (flt(row.rate_of_depreciation) / 100)
        )
...
def update_existing_asset(asset, remaining_qty):
    ...
    for finance_book in asset.get("finance_books"):
        value_after_depreciation = flt(
            (finance_book.value_after_depreciation * remaining_qty)
            / asset.asset_quantity
        )
        expected_value_after_useful_life = flt(
            (finance_book.expected_value_after_useful_life * remaining_qty)
            / asset.asset_quantity
        )
        frappe.db.set_value(
            "Asset Finance Book",
...
def create_new_asset_after_split(asset, split_qty):
    ...
    for finance_book in new_asset.get("finance_books"):
        ...
        finance_book.expected_value_after_useful_life = flt(
            (finance_book.expected_value_after_useful_life * split_qty)
            / asset.asset_quantity
        )
