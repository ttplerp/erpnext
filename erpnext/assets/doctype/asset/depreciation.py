...
def reset_asset_value_for_scrap_sales(asset_name, posting_date):
    ...
    if frappe.db.get_value("Company", asset.company, "reset_asset_value"):
        ...
        if date_diff(posting_date, nowdate()) > 1 and not frappe.db.get_value(
            "Company", asset.company, "allow_back_date_scrapping"
        ):
            frappe.throw(
                _(
                    "Asset scrapping and sales is not allowed for back dates {0}. Scrapping date should be {1}"
                ).format(posting_date, getdate(now()))
            )

        reverse_start_date = posting_date

    if getdate(posting_date) < getdate(
    ...
    if schedules:
        ...
        for i in schedules:
            ...
            je.submit()
        if total_amount > 0:
            value_after_depreciation, finance_book_name = frappe.db.get_value(
                "Asset Finance Book",
                {"asset_sub_category": asset.asset_sub_category, "parent": asset.name},
                ["value_after_depreciation", "name"],
            )
			...
			""".format(
                    value_after_depreciation, finance_book_name, asset.name
                )
            )
    # Pro rating of asset value upon Scrapping Asset in Middle of month written by Thukten
    if (
        cint(frappe.db.get_value("Company", asset.company, "pro_rate_asset_value")) == 1
        and cint(frappe.db.get_value("Company", asset.company, "reset_asset_value"))
        == 0
    ):
        ...
        if posting_date != get_first_day(posting_date):
            ...
            no_of_days_in_month = date_diff(
                get_last_day(getdate(posting_date)),
                get_first_day(getdate(posting_date)),
            )
        if pro_rate_days > 1:
            dtl = frappe.db.sql(
            ...
            if dtl[0].journal_entry:
                frappe.throw(
                    _(
                        "Reversal entry for the depreciation schedule date {0} has not happened"
                    ).format(get_last_day(posting_date))
                )
            pro_rate_depreciation_amount = (
                flt(dtl[0].depreciation_amount) / dtl[0].no_of_days_in_a_schedule
            ) * flt(pro_rate_days)
            pro_rate_depreciation_income_tax = (
                flt(dtl[0].income_depreciation_amount) / dtl[0].no_of_days_in_a_schedule
            ) * flt(pro_rate_days)

            pro_accumulated_depreciation_amount = flt(
                dtl[0].accumulated_depreciation_amount
                - dtl[0].depreciation_amount
                + pro_rate_depreciation_amount
            )
            pro_accumulated_depreciation_income_tax = flt(
                dtl[0].income_accumulated_depreciation
                - dtl[0].income_depreciation_amount
                + pro_rate_depreciation_income_tax
            )
            if pro_rate_depreciation_amount <= 0:
                frappe.throw(
                    _(
                        " Pro Rate depreciation amount is {}, It should be greater than 0"
                    ).format(pro_rate_depreciation_amount)
                )
            ...
            je.branch = asset.branch
            je.remark = "Depreciation Entry against {0} worth {1}".format(
                asset_name, pro_rate_depreciation_amount
            )
            ...
            frappe.db.set_value(
                "Depreciation Schedule", dtl[0].name, "schedule_date", posting_date
            )
            frappe.db.set_value(
            ...
            frappe.db.set_value(
                "Depreciation Schedule", dtl[0].name, "journal_entry", je.name
            )
            frappe.db.set_value(
                "Depreciation Schedule",
                dtl[0].name,
                "depreciation_amount",
                flt(pro_rate_depreciation_amount),
            )
            frappe.db.set_value(
                "Depreciation Schedule",
                dtl[0].name,
                "income_depreciation_amount",
                flt(pro_rate_depreciation_income_tax),
            )
            frappe.db.set_value(
                "Depreciation Schedule",
                dtl[0].name,
                "accumulated_depreciation_amount",
                flt(pro_accumulated_depreciation_amount),
            )
            frappe.db.set_value(
                "Depreciation Schedule",
                dtl[0].name,
                "income_accumulated_depreciation",
                flt(pro_accumulated_depreciation_income_tax),
            )
            frappe.db.set_value(
                "Asset Finance Book",
                finance_book_name,
...
def scrap_asset(asset_name, scrap_date=None):
    ...
    reset_asset_value_for_scrap_sales(
        asset_name, scrap_date if scrap_date else nowdate()
    )
...
def get_gl_entries_on_asset_regain(asset, selling_amount=0, finance_book=None):
    ...
    if profit_amount:
        get_profit_gl_entries(
            profit_amount, gl_entries, disposal_account, depreciation_cost_center
        )
...
def get_gl_entries_on_asset_disposal(asset, selling_amount=0, finance_book=None):
    ...
    (
        ...
        depreciation_cost_center,
    ) = get_disposal_account_and_cost_center(asset.company)

    profit_amount = flt(selling_amount) - flt(value_after_depreciation)
    disposal_account = (
        loss_disposal_account if flt(profit_amount) < 0 else gain_disposal_account
    )
    if profit_amount:
        get_profit_gl_entries(
            profit_amount, gl_entries, disposal_account, depreciation_cost_center
...
def get_disposal_account_and_cost_center(company):
    (
        ...
        "Company",
        company,
        ["loss_disposal_account", "gain_disposal_account", "depreciation_cost_center"],
    )

    if not gain_disposal_account or not loss_disposal_account:
        frappe.throw(
            _("Please set 'Gain/Loss Account on Asset Disposal' in Company {0}").format(
                company
            )
        )
    if not depreciation_cost_center:
        frappe.throw(
            _("Please set 'Asset Depreciation Cost Center' in Company {0}").format(
                company
            )
        )
