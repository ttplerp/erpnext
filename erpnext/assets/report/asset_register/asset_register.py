...
def validate_filters(filters):
    if not filters.fiscal_year:
        ...
        filters.year_start_date = getdate(filters.fiscal_year+"-01-01")
        filters.year_end_date = getdate(filters.fiscal_year+"-12-31")
    '''
...
def get_depreciation_details(filters):
    query= """
        SELECT
            ...
            SUM(CASE
                ...
                ELSE 0
            END) AS depreciation_income_tax
        FROM `tabAsset` a, `tabDepreciation Schedule` as ds
        WHERE a.name= ds.parent and ds.schedule_date <= '{to_date}'
        AND (IFNULL(ds.journal_entry,'') != '' OR IFNULL(ds.depreciation_entry,'') != '')
        AND a.cost_center = '{cost_center}'
        GROUP BY ds.parent
    """.format(from_date=filters.from_date, to_date=filters.to_date, fiscal_year = filters.fiscal_year, cost_center=filters.cost_center)
    ...
    query_two= """
        SELECT
            ...
            SUM(ds.depreciation_amount) AS dep_total_next_year
        FROM `tabAsset` a, `tabDepreciation Schedule` AS ds
        WHERE a.name=ds.parent and YEAR(ds.schedule_date) = '{fiscal_year}' 
        AND (SELECT status FROM `tabAsset` WHERE name = ds.parent) IN ('Submitted','Partially Depreciated')
        AND a.cost_center = '{cost_center}'
        GROUP BY ds.parent
    """.format(fiscal_year = str(int(filters.fiscal_year)+1), cost_center=filters.cost_center)
