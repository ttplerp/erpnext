...
# 						tax.total = flt(self.doc.net_total, tax.precision("total"))
# 			else:
# 				if self.doc.doctype in ("Sales Order", "Sales Invoice", "Delivery Note"):
# 					if frappe.db.get_value("Customer", self.doc.customer, "territory") != "Bhutan":
# 						if tax.is_gst == 1:
# 							tax.add_deduct_tax = "None"
...
# 	# 	tax.item_wise_tax_detail[key] = [tax_rate, flt(item_wise_tax_amount)]
	...
	# 	# DO NOT mutate tax.item_wise_tax_detail
	# 	item_tax_detail = {}

	# 	if tax.item_wise_tax_detail:
	# 		if isinstance(tax.item_wise_tax_detail, str):
	# 			item_tax_detail = json.loads(tax.item_wise_tax_detail)
	# 		else:
	# 			# already dict, copy it
	# 			item_tax_detail = dict(tax.item_wise_tax_detail)

	# 	if item_tax_detail.get(key):
	# 		item_wise_tax_amount += item_tax_detail[key][1]

	# 	item_tax_detail[key] = [tax_rate, flt(item_wise_tax_amount)]

	# 	# Convert BACK to JSON STRING
	# 	tax.item_wise_tax_detail = json.dumps(item_tax_detail)
...
#         # Ensure item_tax_map is always a dict
#         if not tax.item_wise_tax_detail:
#             item_tax_map = {}
#         elif isinstance(tax.item_wise_tax_detail, dict):
#             item_tax_map = tax.item_wise_tax_detail
#         else:
#             try:
...
class calculate_taxes_and_totals(object):
	...
	# 	if not tax.item_wise_tax_detail:
	# 		tax.item_wise_tax_detail = {}
	# 	elif isinstance(tax.item_wise_tax_detail, str):
	...
	def set_item_wise_tax(self, item, tax, tax_rate, current_tax_amount):
		...
		# Step 1: Always work with dict internally
		item_tax_detail = {}
		...
		if tax.item_wise_tax_detail:
			if isinstance(tax.item_wise_tax_detail, str):
				...
				item_tax_detail = tax.item_wise_tax_detail.copy()
		...
		# Step 3: Store back safely
		# POS calculation can accept dict, but DB must be string
		if self.doc.doctype == "Sales Invoice" and self.doc.get("is_consolidated"):
			tax.item_wise_tax_detail = json.dumps(item_tax_detail)
		else:
			tax.item_wise_tax_detail = item_tax_detail	
...
def get_itemised_tax(taxes, with_tax_account=False):
	...
	for tax in taxes:
		...
		# FIX: support dict or json string
		if isinstance(tax.item_wise_tax_detail, str):
			item_tax_map = json.loads(tax.item_wise_tax_detail)
		elif isinstance(tax.item_wise_tax_detail, dict):
			item_tax_map = tax.item_wise_tax_detail
