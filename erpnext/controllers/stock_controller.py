...
class StockController(AccountsController):
            ...
            if not sl_dict.get(dimension.target_fieldname) and dimension.fetch_from_parent:
                ...
                if not sl_dict.get(dimension.target_fieldname):
                    fieldname = frappe.get_cached_value(
                        "DocField",
                        {"parent": self.doctype, "options": dimension.fetch_from_parent},
                        "fieldname",
                    )
                    ...
                    if not fieldname:
                        fieldname = frappe.get_cached_value(
                            "Custom Field",
                            {"dt": self.doctype, "options": dimension.fetch_from_parent},
                            "fieldname",
                        )
    ...
    def make_sl_entries(
        self, sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False
    ):
    ...
    def validate_warehouse(self):
        ...
        warehouses = list(
            set(d.warehouse for d in self.get("items") if getattr(d, "warehouse", None))
        )
    ...
    def validate_qi_presence(self, row):
        ...
        if not row.quality_inspection:
            ...
            if self.docstatus == 1:
                frappe.throw(
                    _(msg), title=_("Inspection Required"), exc=QualityInspectionRequiredError
                )
            else:
    ...
    def validate_qi_rejection(self, row):
        ...
        if qa_status == "Rejected":
            link = frappe.utils.get_link_to_form("Quality Inspection", row.quality_inspection)
            msg = (
                f"Row #{row.idx}: Quality Inspection {link} was rejected for item {row.item_code}"
            )
            if action == "Stop":
                frappe.throw(
                    _(msg), title=_("Inspection Rejected"), exc=QualityInspectionRejectedError
                )
            else:
                frappe.msgprint(_(msg), alert=True, indicator="orange")
    ...
    def validate_internal_transfer(self):
        if (
            self.doctype
            in ("Sales Invoice", "Delivery Note", "Purchase Invoice", "Purchase Receipt")
