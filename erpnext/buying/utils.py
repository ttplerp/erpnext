...
def update_last_purchase_rate(doc, is_submit) -> None:
    ...
    for d in doc.get("items"):
        ...
        if last_purchase_details and (
            doc.get("docstatus") == 2
            or last_purchase_details.purchase_date > this_purchase_date
        ):
        ...
        frappe.db.set_value(
            "Item", d.item_code, "last_purchase_rate", flt(last_purchase_rate)
        )
...
def validate_for_items(doc) -> None:
    ...
    if (
        ...
        and not cint(
            frappe.db.get_single_value("Buying Settings", "allow_multiple_items") or 0
        )
    ):
...
def check_on_hold_or_closed_status(doctype, docname) -> None:
    ...
    if status in ("Closed", "On Hold"):
        frappe.throw(
            _("{0} {1} status is {2}").format(doctype, docname, status),
